...
RingBufferWiFi::RingBufferWiFi(unsigned int size)
{
	_size = size;
...
RingBufferWiFi::~RingBufferWiFi() {}

void RingBufferWiFi::reset()
{
	ringBufP = ringBuf;
}

void RingBufferWiFi::init()
{
	ringBufP = ringBuf;
	memset(ringBuf, 0, _size+1);
}

void RingBufferWiFi::push(char c)
{
	*ringBufP = c;
	ringBufP++;
...
bool RingBufferWiFi::endsWith(const char* str)
{
...
void RingBufferWiFi::getStr(char * destination, unsigned int skipChars)
{
...
void RingBufferWiFi::getStrN(char * destination, unsigned int skipChars, unsigned int num)
{
	int len = ringBufP-ringBuf-skipChars;
